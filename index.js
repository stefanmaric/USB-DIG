...
import { ContentInfo, SignedData, Certificate, getCrypto } from 'pkijs'
import { stringToArrayBuffer } from 'pvutils'

document.getElementById('pdfFile').addEventListener('change', handleFileBrowse, false)
document.getElementById('caBundle').addEventListener('change', handleCABundle, false)

async function getSignatures(pdf) {
  const signatures = []
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    const sigField = annotations.find(field => field.fieldType === 'Sig')

    if (sigField) signatures.push(sigField)
  }
...
async function verifyPDFSignature(buffer) {
  try {
    ...
    const pdf = await window.pdfjsLib.getDocument(view).promise
    const signatures = await getSignatures(pdf)

    if (!signatures.length) throw new Error('The PDF document has no signatures')

    const { ByteRange: byteRange, Contents: contents } = signatures[0].fieldValue
    ...
    const cmsResult = await cmsSignedSimp.verify({
      signer: 0,
    ...
    if (cmsResult === false) throw new Error('Signature verification failed')
    ...
    const contentDiggest = await crypto.digest(
      { name: shaAlgorithm },
    ...
    for (let j = 0; j < cmsSignedSimp.signerInfos[0].signedAttrs.attributes.length; j++) {
      if (cmsSignedSimp.signerInfos[0].signedAttrs.attributes[j].type === '1.2.840.113549.1.9.4') {
        messageDigest =
          cmsSignedSimp.signerInfos[0].signedAttrs.attributes[j].values[0].valueBlock.valueHex
        break
      }
    }
    ...
    const view2 = new Uint8Array(contentDiggest)
